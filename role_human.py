...
def install_human_windows(node,user,control_ipv4_addr,password):
    stdout=[]
    stderr=[]
    exit_status=[]
...
def deploy_human(obj):
    ...
    if user == "Administrator":
        return install_human_windows(node,user,control_ipv4_addr,password)
    elif user == "ubuntu":
        return install_human_linux(node,user,control_ipv4_addr,password)
    else:
